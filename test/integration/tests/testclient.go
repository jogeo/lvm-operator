...
func NewTestClient(testName string) *TestClient {
	...
	if err != nil {
		e2e.Failf("Failed to load kubeconfig: %v", err)
	}
	...
	if err != nil {
		e2e.Failf("Failed to create kubernetes client: %v", err)
	}
...
func (tc *TestClient) SetupProject() error {
	...
	if err := tc.Create(tc.ctx, ns); err != nil {
		e2e.Logf("Failed to create namespace %s: %v", tc.Namespace(), err)
		return err
	}

	e2e.Logf("Created test namespace: %s", tc.Namespace())
...
func (cb *CommandBuilder) execute() (string, error) {
	...
	// Add namespace if needed and not explicitly set
	...
	if err != nil {
		e2e.Logf("Command failed: oc %s\nOutput: %s\nError: %v",
			strings.Join(cmdArgs, " "), string(output), err)
...
func (tc *TestClient) DeleteNamespace() error {
	...
	if err := tc.Delete(tc.ctx, ns); err != nil {
		e2e.Logf("Failed to delete namespace %s: %v", tc.namespace, err)
		return err
	}

	e2e.Logf("Deleted test namespace: %s", tc.namespace)
	return nil
}
...
func (tc *TestClient) CreateFromYAML(yamlPath string) error {
	...
	if err != nil {
		e2e.Logf("Failed to create from YAML %s: %v\nOutput: %s", yamlPath, err, string(output))
		return err
	}

	e2e.Logf("Created resources from %s", yamlPath)
	return nil
}
	...
	for {
		...
		default:
			...
			if err := tc.Get(tc.ctx, key, pod); err != nil {
				e2e.Logf("Failed to get pod %s/%s: %v", namespace, podName, err)
			...
			for _, condition := range pod.Status.Conditions {
				if condition.Type == corev1.PodReady && condition.Status == corev1.ConditionTrue {
					e2e.Logf("Pod %s/%s is ready", namespace, podName)
					return nil
				}
			}
