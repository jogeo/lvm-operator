...
1. Start with one test file (e.g., `lvms.go`)
2. Use Phase 1 migration (drop-in replacement)
3. Verify tests pass
4. Gradually adopt Phase 2 (type-safe APIs) where it makes sense
5. Remove `github.com/openshift/origin/test/extended/util` from imports
6. Update `go.mod` to remove the dependency
